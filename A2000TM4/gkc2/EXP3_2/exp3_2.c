...
bool is_GET(uint8_t *str);
bool is_SET(uint8_t *str);
...
int main(void)
{
	...
	while (1)
	{
		...
		if (cmd_state == 1)
		{
			cmd_state = 0;
			uart_receive_len = 0;
			// UARTStringPutNOBlocking(UART0_BASE, buffer);
			// 根据实际要求添加处理代码

			if (is_SET(buffer)) // 判断是否为SET
			{
				run_SET();
			}

			else if (is_INC(buffer)) // 判断是否为INC
			{
				run_INC();
			}

			else if (is_GET(buffer)) // 判断是否是GET
			{
				run_GET();
			}
			else
