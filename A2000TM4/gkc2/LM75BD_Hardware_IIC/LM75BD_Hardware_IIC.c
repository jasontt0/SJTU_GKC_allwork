...
// Description: 本示例如何每隔1s读取温度传感器LM75BD的温度值
      ...
      if (clock1s_flag == 1)      // 检查1秒定时是否到
        {
          ...
          i16Temperature1 = GetTemputerature(LM75BD_ADR2);   //0X48
          i16Temperature2 = GetTemputerature(LM75BD_ADR1);   //0X4F
...
//
//******************************************************************************************
void SysTick_Handler(void)       // 定时周期
{
  // 0.1秒钟软定时器计数
  if (++clock1s >= V_T1s)
    {
