...
// File name: exp5_recv.c
// Description: 本程序采用测频率法实现方波频率的测量,测定频率范围【300，4000】。
//              Timer1
// 首先将待测方波信号连接到T0CCP0(PL4)引脚；
// 每次按下A2000TM4C底板按键1(SW1)后,程序开始测量方波信号的频率，测量完成后数码管显示测量结果,单位为Hz;
// 如果测试频率不在【300，4000】范围内容，则显示Err。
// 注意事项：待测方波信号高电平不宜高于3.3V，低电平不宜低于0V，并且一定要与底板共地
// Author:	上海交通大学电子工程系实验教学中心
// Version: 1.0.0.20210512 
// Date：2021-05-12
// History：
//
//*******************************************************************************************************
...
// 记录方波上升沿捕获次数
volatile uint8_t g_ui8Counter = 0;

// 捕获结束标志, 0表示捕获没有结束，1表示捕获结束
volatile uint8_t g_ui8INTStatus = 0;

// 记录连续两次上升沿捕获时定时器的计数值
volatile uint32_t g_ui32Ticks[2];
...
//    UARTprintf("%d,%d,%d\n",123,456,789);
    ...
    {	
        ...
        if(g_ui8INTStatus == 1)   // 捕获结束，开始计算频率
        {
            g_ui8INTStatus = 0;
...
//void UART_Init(void){
//    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOA);//GPIOA使能
//    GPIOPinConfigure(GPIO_PA0_U0RX);
//    GPIOPinConfigure(GPIO_PA1_U0TX);//配置引脚复用
//    SysCtlPeripheralEnable(SYSCTL_PERIPH_UART0);//串口使能
//    UARTClockSourceSet(UART0_BASE, UART_CLOCK_PIOSC);//配置串口时钟源为精确内部振荡器(16MHz)
//    GPIOPinTypeUART(GPIO_PORTA_BASE, GPIO_PIN_0 | GPIO_PIN_1);//引脚映射
//    UARTStdioConfig(0, 115200, 16000000);//配置波特率，这个函数只适用于UART0
//}
