...
{
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOC);  //Cnable PortC
    while(!SysCtlPeripheralReady(SYSCTL_PERIPH_GPIOC));
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOK);  //Cnable PortK
    while(!SysCtlPeripheralReady(SYSCTL_PERIPH_GPIOK));
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOB);  //Cnable PortB
    while(!SysCtlPeripheralReady(SYSCTL_PERIPH_GPIOB));
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOE);  //Cnable PortE
...
static uint8_t get_data_from_ROM(uint8_t inverse )
{
    uint8_t i;
    uint8_t ret_data=0;
    for(i=0; i<8; i++)
...
void get_and_write_16x16(unsigned long fontaddr,uint8_t page,uint8_t column,uint8_t inverse)
{
    ...
    for(j=0; j<2; j++)
    {
        ...
        for(i=0; i<16; i++ )
        {
            disp_data=get_data_from_ROM(inverse);
