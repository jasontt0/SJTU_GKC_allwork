...
#define V_T1500ms 10 // 1.5s软件定时器溢出值，75个20ms
...
void UARTInit(void)
{
    ...
    // UARTStringPut(UART6_BASE, (const char *)"\r\nHello, 2A!\r\n");
...
{
    /*
    display_GB2312_string(1, 1, "电压：", 0); // ACT0界面
    display_GB2312_string(1, 41, volt1, 0);
    display_GB2312_string(1, 49, ".", 0);
    display_GB2312_string(1, 57, volt2, 0);
    display_GB2312_string(1, 65, volt3, 0);
    display_GB2312_string(1, 73, volt4, 0);
...
{
    Get_Frequency();
    temperature = ((ui32Freq - 300) * 40) / 370;
    (*temperature1)++;
