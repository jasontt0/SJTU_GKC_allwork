...
int main(void)
{
  uint8_t current_PJ=2;	//读取PJ0输入
	uint8_t previous_PJ=2;	//PJ1前一状态
...
void run_mode(void)
{
	...
	if(mode==0) counter_for_100ms();	//功能1：频率为100ms的计时和显示
	if(mode==1) flash_light_run_right();	//功能2：走马灯向右
	if(mode==2) flash_light_run_left();	//功能2：走马灯向左
