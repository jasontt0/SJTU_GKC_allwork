...
void UU_counter_for_100ms(void);	//加计时
void UU_reset_counter(void);
void AA_counter_for_100ms(void);	//减计时
void AA_reset_counter(void);
...
void switch_run_stop(void);	//转变不同工作模式
void switch_AA_UU(void);
...
int main(void)
{
  ...
  TM1638_Init();	          // 初始化TM1638
	...
	if(key_code==1) switch_run_stop();
	if(key_code==3) switch_AA_UU();
		
	run_mode();		
		
	if(key_code==2 && AA) AA_reset_counter();
	else
		if(key_code==2 && (!AA))UU_reset_counter();
...
//****************************************************************************
//减计时
void AA_counter_for_100ms(void)
{
	if (clock100ms_flag == 1)      // 检查0.1秒定时是否到
		{
			clock100ms_flag		= 0;// 每0.1秒累加计时值在数码管上以十进制显
          
			--test_counter;
			if (test_counter <=1) test_counter = 9999;
...
//****************************************************************************
//加计时
void UU_counter_for_100ms(void)
{
	if (clock100ms_flag == 1)      // 检查0.1秒定时是否到
		{
			clock100ms_flag		= 0;// 每0.1秒累加计时值在数码管上以十进制显
...
void run_mode(void)
{
	...
	if(run && AA)
	{
		AA_counter_for_100ms();	//功能1：频率为100ms的计时和显示
		led[7]=1;
		led[0]=1;//由于按键不灵敏，用于指示
		...
		if(run && (!AA))
		{
			UU_counter_for_100ms();	//功能1：频率为100ms的计时和显示
			led[7]=1;
			led[0]=0;//由于按键不灵敏，用于指示
