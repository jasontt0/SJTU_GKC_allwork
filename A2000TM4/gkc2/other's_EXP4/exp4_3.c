...
void initial_act0(void);     //act0画面初始化
void initial_act1(void);     //act1画面初始化
void initial_act2(void);     //act2画面初始化
void initial_act3(void);     //act3画面初始化
void initial_act4(void);     //act4画面初始化

void act0_ui_state_proc(uint8_t ui_state);      //act0画面下根据ui_state向各子过程的分流
void act1_ui_state_proc(uint8_t ui_state);      //act1画面下根据ui_state向各子过程的分流
void act2_ui_state_proc(uint8_t ui_state);      //act2画面下根据ui_state向各子过程的分流
void act3_ui_state_proc(uint8_t ui_state);      //act3画面下根据ui_state向各子过程的分流
void act4_ui_state_proc(uint8_t ui_state);      //act4画面下根据ui_state向各子过程的分流，当然本问题中act4画面下没有向子过程的分流

void act0_ui_proc0(void);                             //act0初始没有光标仅显示字
void act0_ui_proc_set(void);                         //处理act0的“设置”

void act1_ui_proc0(void);                            //act1初始没有光标仅显示字
void act1_ui_proc_workmode(void);            //处理act1的“工作模式”
void act1_ui_proc_workpara(void);              //处理act1的“工作参数”
void act1_ui_proc_return(void);                  //处理act1的“返回”

void act2_ui_proc0(void);                           //act2初始没有光标仅显示字
void act2_ui_proc_modeX(void);                 //处理act2的“模式x”
void act2_ui_proc_confirm(void);               //处理act2的“确定”
void act2_ui_proc_cancel(void);                 //处理act2的“取消”

void act3_ui_proc0(void);                          //act3初始没有光标仅显示字
void act3_ui_proc_unit(void);                    //处理act3的“(个位数)”
void act3_ui_proc_tenths(void);                //处理act3的“(十分位数)”
void act3_ui_proc_confirm(void);              //处理act3的“确定”
void act3_ui_proc_cancel(void);                //处理act3的“取消”

void in_de_num ( uint8_t w, /*unsigned*/ char* actstring );          //加减个/十分位数字
void in_de_capletter ( uint8_t w, /*unsigned*/ char* actstring );   //加减“模式X”中的大写字母
...
char initial_unit[10], initial_tenths[10];         //用于保存某次则进入act3画面时初始的个位和十分位
...
{
	clear_screen();
	//display_GB2312_string(1, 1, "                          ", 0);//第一行空白
	//display_GB2312_string(3, 1, "   ", 0);//第二行开头空白
	display_GB2312_string(3, (3-1)*8+1, str[0], 0);
	//display_GB2312_string(3, (8-1)*8+1, "     ", 0);//第二行中间空白
	...
	//display_GB2312_string(3, (16-1)*8+1, "  ", 0);//第二行最后空白
	//display_GB2312_string(5, 1, "                          ", 0);//第三行空白
	display_GB2312_string(7, 1, "设置", 0);
	//display_GB2312_string(7, (5-1)*8+1, "                   ", 0);//第四行最后空白
}
...
void initial_act1(void)
{
	...
	//display_GB2312_string(3, (11-1)*8+1, "          ", 0);//第二行最后空白
	//display_GB2312_string(5, 1, "   ", 0);//第三行开头空白
  display_GB2312_string(5, (3-1)*8+1, "工作参数", 0);
	//display_GB2312_string(5, (11-1)*8+1, "          ", 0);//第三行最后空白
	//display_GB2312_string(7, 1, "                    ", 0);//第四行开头空白
...
void initial_act2(void)
{
	...
	//display_GB2312_string(3, 16, "  ", 0);//第二行最后空白
  //display_GB2312_string(5, 1, "                          ", 0);//第三行空白
...
void initial_act3(void)
{
	...
	strcpy(initial_tenths, str[2]);//先保存初始的个位和十分位内容
...
{
	if(ui_prestate == 0){ui_prestate = 1; clear_flags();}//从状态0到状态1对标志位的影响不算
	if(key_RIGHT_flag)
  {
      key_RIGHT_flag=0;
  }
  ...
  else if (key_ENTER_flag)//跳转到act1
	{
      key_ENTER_flag=0;
		  key_ANY_flag = 0;
		  //clear_screen();
...
void act1_ui_proc_workmode(void)
{
  ...
  else if (key_INCREASE_flag)
  {
	...
	else if (key_ENTER_flag)//跳转到act2
	{
      key_ENTER_flag=0;
		  key_ANY_flag = 0;
		  initial_act2();
...
void act1_ui_proc_workpara(void)
{
  ...
  else if (key_INCREASE_flag)
  {
	...
	else if (key_ENTER_flag)//跳转到act3
	{
      key_ENTER_flag=0;
		  key_ANY_flag = 0;
		  initial_act3();
...
void act1_ui_proc_return(void)
{
  ...
  else if (key_INCREASE_flag)
  {
	...
	else if (key_ENTER_flag)//跳转到act0
	{
      key_ENTER_flag=0;
		  key_ANY_flag = 0;
		  initial_act0();
...
void act2_ui_proc_confirm(void)
{
  ...
  else if (key_INCREASE_flag)
  {
	...
	else if (key_ENTER_flag)//跳转到act1，且模式X设置成功，需更新初始值
	{
...
void act2_ui_proc_cancel(void)
{
  ...
  else if (key_INCREASE_flag)
  {
	...
	else if (key_ENTER_flag)//跳转到act1，且模式X设置失败，需复原为初始值
	{
      key_ENTER_flag=0;
		  key_ANY_flag = 0;
...
void act3_ui_proc_confirm(void)
{
  ...
  else if (key_INCREASE_flag)
  {
	...
	else if (key_ENTER_flag)//跳转到act1或act4 !
	{
...
void act3_ui_proc_cancel(void)
{
  ...
  else if (key_INCREASE_flag)
  {
	...
	else if (key_ENTER_flag)//跳转到act1，且个位和十分位设置失败，均需复原为初始值
	{
      key_ENTER_flag=0;
		  key_ANY_flag = 0;
		  strcpy(str[1], initial_unit);
...
void in_de_num ( uint8_t w, /*unsigned*/ char* actstring )    //w=1: increase; w=2: decrease; 数字增减
{
	if ( w==1 )
	{
		(* actstring)++;
...
void in_de_capletter ( uint8_t w, /*unsigned*/ char* actstring )    //w=1: increase; w=2: decrease; 字母增减
{
	actstring += 4; //注意此处含汉字的字符串中一个汉字占数组的两项 ！
	if ( w==1 )
	{
		(* actstring)++;
...
void SysTick_Handler(void)       // 定时周期为20ms
{
	...
	if (key_code == 0)
	{
		...
		{
			NOKEY_clock10s_flag = 1; //当10s到时，溢出标志置1
			clock10s = 0;
		}
	}
	else clock10s = 0;
	...
	/*if (pre_key_code != 6 && key_code==6){clock10s_flag = 0; key_RIGHT_flag =1;}                                      //更新标志key_RIGHT_flag
	else if ( pre_key_code != 4 && key_code==4 ){clock10s_flag = 0; key_LEFT_flag =1;}                                //更新标志key_LEFT_flag
	else if ( pre_key_code != 2 && key_code==2 ){clock10s_flag = 0; key_INCREASE_flag = 1;}                      //更新标志key_INCREASE_flag
	else if ( pre_key_code != 8 && key_code==8 ){clock10s_flag = 0; key_DECREASE_flag = 1;}                     //更新标志key_DECREASE_flag
	else if ( pre_key_code != 5 && key_code==5 ){clock10s_flag = 0; key_ENTER_flag =1;}                             //更新标志key_ENTER_flag
	...
	else if ( ++clock10s >= V_T10s ){NOKEY_clock10s_flag = 1; clock10s_flag = 1; clock10s = 0;}                 //更新标志NOKEY_clock10s_flag
