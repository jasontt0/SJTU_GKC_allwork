...
//
//*************************************************************************************
#define SYSTICK_FREQUENCY		50		// SysTick频率为50Hz，即循环定时周期20ms        *改动1，原100

#define V_T40ms	 2                      // 40ms软件定时器溢出值，2个20ms          *改动2，原1
#define V_T100ms 5                      // 0.1s软件定时器溢出值，5个20ms          *改动3，原10
...
*/
						...
						digit[4] = average0 / 1000; 	  // 显示CH0/PE3的ADC采样值千位数
            digit[5] = average0 / 100 % 10;  // 显示CH0/PE3的ADC采样值百位数
            digit[6] = average0 / 10 % 10;   // 显示CH0/PE3的ADC采样值十位数
            digit[7] = average0 % 10;        // 显示CH0/PE3的ADC采样值个位数

            digit[0] = average1 / 1000; 	  // 显示CH1/PE2的ADC采样值千位数
            digit[1] = average1 / 100 % 10;  // 显示CH1/PE2的ADC采样值百位数
            digit[2] = average1 / 10 % 10;   // 显示CH1/PE2的ADC采样值十位数
            digit[3] = average1 % 10;        // 显示CH1/PE2的ADC采样值个位数  
