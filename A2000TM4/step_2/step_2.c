...
int main(void)
{
	...
	while (1)
	{
		...
		if (DAC6571_flag == 1) // 检查DAC电压是否要变
		{
			...
			DAC6571_code = 4.3563 * target_current / 10 - 36.4175;
			DAC6571_code = 0.7789 * DAC6571_code + 2.8027 + 2;
			DAC6571_code = 0.9436 * DAC6571_code - 0.0618;
			// if (target_current < 150)
			// {
			// 	DAC6571_code += 0.5;
			// }
			// else
		...
		if (clock40ms_flag == 1) // 检查40ms秒定时是否到
		{
			...
			if (output_flag)
				value_output = average0;
			else
				value_output = average1;
...
void SysTick_Handler(void) // 定时周期为20ms
{
	...
	if (++clock200ms >= V_T200ms)
	{
		clock200ms_flag = 1; // 当0.2秒到时，溢出标志置1
		clock200ms = 0;
	}
